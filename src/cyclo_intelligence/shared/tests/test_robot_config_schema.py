...
def test_sh5_state_and_action_dimensions_match_layout():
    ...
    # Existing HX5 checkpoints and datasets use the audited 54-D arm+hand
    # contract. Mission Canvas-only head/lift channels must not change it.
    assert state_dim == 54
    assert action_dim == 54

    bt_state_dim = sum(
        len(cfg["joint_names"])
        for cfg in robot_schema.get_bt_state_groups(section).values()
    )
    bt_action_dim = sum(
        len(cfg["joint_names"])
        for cfg in robot_schema.get_bt_action_groups(section).values()
    )
    assert bt_state_dim == 57
    assert bt_action_dim == 57
    assert robot_schema.get_bt_action_groups(section)["head"]["record"] is False


def test_sh5_is_supported_by_behavior_tree_runtime():
    assert robot_schema.is_bt_supported("ffw_sh5_rev1")
    assert "ffw_sh5_rev1" in robot_schema.bt_supported_robot_types()


def test_live_sh5_model_receives_head_and_lift_without_changing_policy_layout():
    section = robot_schema.load_robot_section("ffw_sh5_rev1")

    assert robot_schema.get_visualization_joint_state_topics(section) == [
        "/arm_hand/joint_states", "/joint_states",
    ]
    ...
    assert sum(len(group["joint_names"])
               for group in robot_schema.get_action_groups(section).values()) == 54


def test_visualization_feedback_deduplicates_topics_and_excludes_non_joint_state():
    section = {
        "observation": {"state": {
            "arms": {"topic": "/robot/measured_joints"},
            "base": {"topic": "/odom", "msg_type": "nav_msgs/msg/Odometry"},
        }},
        "behavior_tree": {"state": {
            "head": {"topic": "/robot/measured_joints"},
            "lift": {"topic": "/lift/measured_joints"},
            "tactile": {"topic": "/hand_pressure", "msg_type": "robotis_interfaces/msg/HandPressures"},
        }},
    }

    assert robot_schema.get_visualization_joint_state_topics(section) == [
        "/robot/measured_joints", "/lift/measured_joints",
    ]
    assert robot_schema.get_joint_state_topics(section) == ["/robot/measured_joints"]


def test_sg2_live_visualization_feedback_is_preserved():
    section = robot_schema.load_robot_section("ffw_sg2_rev1")
    assert robot_schema.get_visualization_joint_state_topics(section) == (
        robot_schema.get_joint_state_topics(section)
    )
