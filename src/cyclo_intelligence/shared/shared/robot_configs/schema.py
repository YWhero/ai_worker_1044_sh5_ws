...
around what a VLA dataset / training pipeline cares about:
  ...
  behavior_tree:                           # optional, policy-layout neutral
    state:
      <group_name>: {topic, msg_type, joint_names}
    action:
      <group_name>: {topic, msg_type, joint_names}
  recording:
...
def get_bt_state_groups(section: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    """State groups visible to Behavior Tree actions.

    Policy observation layouts must remain checkpoint-compatible.  Robots can
    therefore expose extra JointState feedback to Action Canvas under
    ``behavior_tree.state`` without changing the policy state dimension.
    """
    result = get_state_groups(section)
    extras = (section.get("behavior_tree") or {}).get("state") or {}
...
def get_bt_action_groups(section: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    """Command groups visible to Behavior Tree actions.

    ``behavior_tree.action`` augments the policy action groups but is never
    consumed by recording, conversion, training, or inference code.
    """
    result = get_action_groups(section)
    extras = (section.get("behavior_tree") or {}).get("action") or {}
    for name, cfg in extras.items():
        if not isinstance(cfg, dict) or "topic" not in cfg:
            continue
        result[name] = {
...
def get_visualization_joint_state_topics(section: Dict[str, Any]) -> List[str]:
    """Feedback needed to draw the full URDF, including non-policy joints.

    Head and lift can live in ``behavior_tree.state`` to keep the policy's
    observation vector checkpoint-compatible. The live model still needs
    those measured positions; this does not augment the policy layout.
    """
    return list(dict.fromkeys(
        cfg["topic"]
        for cfg in get_bt_state_groups(section).values()
        if cfg.get("msg_type") == "sensor_msgs/msg/JointState"
    ))
