...
Main image (cyclo_intelligence):
  ...
  build [main|lerobot|vitacformer|groot ...]
                   Build selected images only; do not start or replace containers
...
build_images() {
    ...
    for requested in "$@"; do
        case "$requested" in
            main) services+=("$MAIN_SERVICE") ;;
            lerobot) services+=("$LEROBOT_SERVICE") ;;
            vitacformer) services+=("$VITACFORMER_SERVICE") ;;
            groot) services+=("$GROOT_SERVICE") ;;
            *) echo "Unsupported build target: $requested" >&2; return 2 ;;
        esac
    done
