...
class ConversionConfig:
    ...
    # ``state_mean`` preserves the legacy per-finger means appended to
    # observation.state. ``separate_raw`` is the T-Rex contract: keep every
    # 3x3 taxel in observation.tactile.<side> and store an episode-local
    # no-contact baseline without contaminating the robot state vector.
...
class RosbagToLerobotConverterBase:
    ...
    def __init__(self, config: ConversionConfig, logger=None):
        self.config = config
        self.logger = logger
        if config.tactile_mode not in {"state_mean", "separate_raw"}:
            raise ValueError(
                "tactile_mode must be 'state_mean' or 'separate_raw', got "
    ...
    def _update_config_from_robot_config(self, robot_config: Dict):
        ...
        if "tactile_topics" in robot_config:
