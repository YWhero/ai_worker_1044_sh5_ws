...
: rclcpp::Node("service_bag_recorder")
{
  ...
  record_with_ros_clock_ = this->declare_parameter<bool>("record_with_ros_clock", false);
  if (record_with_ros_clock_ && !this->get_parameter("use_sim_time").as_bool()) {
    throw std::invalid_argument("record_with_ros_clock requires use_sim_time=true");
  }
  RCLCPP_INFO(
    this->get_logger(), "Bag timeline: %s",
    record_with_ros_clock_ ? "ROS simulation clock" : "RMW source timestamp");
  ...
  const rclcpp::MessageInfo & message_info)
{
  ...
  // Preserve the existing physical-recording timeline unless explicitly opted
  // into ROS simulation time. RMW source timestamps use the system clock even
  // when use_sim_time=true; mixing them with simulated state/video headers
  // prevents the converter from aligning actions with observations.
  const auto & rmw_info = message_info.get_rmw_message_info();
  const auto clock = this->get_clock();
  const auto timestamp_ns = rosbag_recorder::recording_timestamp(
    record_with_ros_clock_, clock->ros_time_is_active(),
    record_with_ros_clock_ ? clock->now().nanoseconds() : 0,
    rmw_info.source_timestamp);
  if (!timestamp_ns) {
    RCLCPP_WARN_THROTTLE(
      this->get_logger(), *clock, 5000,
      "Waiting for a nonzero /clock before writing simulation messages");
    return;
  }
  const rclcpp::Time record_timestamp(*timestamp_ns, RCL_ROS_TIME);
  ...
  // Write to bag with lock - double-check writer_ inside lock to prevent race condition
  {
    ...
    writer_->write(serialized_msg, topic, type, record_timestamp);
  }
