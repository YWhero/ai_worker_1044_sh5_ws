...
class JointControl(BaseAction):
    ...
    def from_xml_params(cls, context, name: str, params: dict):
        ...
        if _has_value(params.get('groups')):
            ...
            enable_hands = _as_bool(params.get('enable_hands', False))
            ...
            if enable_hands:
                kwargs['left_hand_positions'] = params.get(
                    'left_hand_positions', None,
                )
                kwargs['right_hand_positions'] = params.get(
                    'right_hand_positions', None,
                )
                left_hand_joint_names = params.get('left_hand_joint_names')
                if _has_value(left_hand_joint_names):
                    kwargs['left_hand_joint_names'] = _parse_groups(
                        left_hand_joint_names,
                    )
                right_hand_joint_names = params.get('right_hand_joint_names')
                if _has_value(right_hand_joint_names):
                    kwargs['right_hand_joint_names'] = _parse_groups(
        ...
        if params.get('state_max_age_sec') is not None:
            kwargs['state_max_age_sec'] = params['state_max_age_sec']
    ...
    def __init__(
        ...
        left_hand_positions: Optional[list[float]] = None,
        right_hand_joint_names: Optional[list[str]] = None,
        right_hand_positions: Optional[list[float]] = None,
        ...
        if (not math.isfinite(self.position_threshold) or self.position_threshold < 0
                or not math.isfinite(self.state_max_age_sec) or self.state_max_age_sec <= 0):
            raise ValueError('JointControl: invalid position threshold or feedback age')
        ...
        if not self._channels:
            raise ValueError(
                ...
                'group (enable_head, enable_arms, enable_hands, enable_lift).'
            )
        ...
        if not math.isfinite(self.duration) or self.duration <= 0:
            raise ValueError('JointControl: duration must be finite and positive')
    ...
    def _build_legacy_channels(
        ...
        *,
        ...
        for enabled, side_groups in (
            (enable_arms, (
                ('arm_left', left_joint_names, left_positions),
                ('arm_right', right_joint_names, right_positions),
            )),
            (enable_hands, (
                ('hand_left', left_hand_joint_names, left_hand_positions),
                ('hand_right', right_hand_joint_names, right_hand_positions),
            )),
        ):
            if not enabled:
                continue
            for group, names, values in side_groups:
                # Explicitly empty sides are deselected in the Canvas. Omitted
                # values (None) preserve the legacy full-group default.
                if values is not None and not _has_value(values):
                    continue
                self._channels.append(self._make_channel(
                    group=group,
                    joint_names=names,
    ...
    def _make_channel(
        ...
        if not all(math.isfinite(value) for value in target_positions):
            raise ValueError(f"JointControl: non-finite target for '{group}'")
        configured_names = self.topic_config.get('joint_order', {}).get(group_key, [])
        if configured_names and any(name not in configured_names for name in resolved_joint_names):
            raise ValueError(f"JointControl: joint outside configured '{group}' group")
    ...
    def _publish_channel(self, channel):
        ...
        duration_ns = round(self.duration * 1_000_000_000)
