...
describe('taskSlice task ownership', () => {
  ...
  test('does not let a legacy generic 15 Hz server echo replace the ACT default', () => {
    ...
    expect(inferenceInfo).toMatchObject({
      serviceType: 'lerobot',
      policyType: 'act',
      inferenceHz: 30,
    });
  ...
  test('defaults initial pose sync settings', () => {
    const inferenceInfo = selectInferenceTaskInfo({ tasks: reducer(undefined, { type: '@@init' }) });
    expect(inferenceInfo.initialPoseSync).toBe(false);
    expect(inferenceInfo.initialPoseSyncDurationS).toBe(5.0);
  });
