...
export default function useJointStateSubscription(
  ...
  const lastJointUpdateRef = useRef({});
  ...
  const handleJointState = useCallback(
    (msg, topicName = '') => {
      const now = Date.now();
      const lastUpdate = lastJointUpdateRef.current[topicName] || 0;
      if (now - lastUpdate < throttleMs) return;
      lastJointUpdateRef.current[topicName] = now;
  ...
  useEffect(() => {
    ...
    const subscribeTopic = (ros, topic, callback) => {
      ...
      sub.subscribe((message) => callback(message, topic.name));
      subs.push(sub);
    };
