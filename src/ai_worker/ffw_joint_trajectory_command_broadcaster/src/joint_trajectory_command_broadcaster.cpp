  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  try {
    ...
    for (const auto & group_name : groups) {
      ...
      if (group_name == "left") {
        group_min_positions_[group_name] = params_.left_min_positions;
        group_max_positions_[group_name] = params_.left_max_positions;
      } else {
        group_min_positions_[group_name] = params_.right_min_positions;
        group_max_positions_[group_name] = params_.right_max_positions;
      }
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  for (const auto & group_name : trajectory_groups_) {
    ...
    const auto & min_positions = group_min_positions_[group_name];
    const auto & max_positions = group_max_positions_[group_name];
    if (min_positions.empty() != max_positions.empty() ||
      (!min_positions.empty() &&
      (min_positions.size() != num_joints || max_positions.size() != num_joints)))
    {
      RCLCPP_ERROR(
        get_node()->get_logger(),
        "Position limits for group '%s' must both contain exactly %zu values.",
    ...
    for (size_t i = 0; i < min_positions.size(); ++i) {
      if (min_positions[i] > max_positions[i]) {
        RCLCPP_ERROR(
          get_node()->get_logger(),
          "Minimum position exceeds maximum for group '%s', joint '%s'.",
          group_name.c_str(), group_joints[i].c_str());
        return CallbackReturn::ERROR;
      }
    }
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  group_min_positions_.clear();
  group_max_positions_.clear();
...
double JointTrajectoryCommandBroadcaster::transform_position(
  const std::string & group_name, const std::string & joint_name,
  size_t joint_index, double position) const
{
  const auto reverse_it = group_reverse_joints_.find(group_name);
  if (reverse_it != group_reverse_joints_.end() &&
    std::find(reverse_it->second.begin(), reverse_it->second.end(), joint_name) !=
    reverse_it->second.end())
  {
    position = -position;
  }

  const auto offsets_it = group_joint_offsets_.find(group_name);
  if (offsets_it != group_joint_offsets_.end() && joint_index < offsets_it->second.size()) {
    position += offsets_it->second[joint_index];
  }

  const auto min_it = group_min_positions_.find(group_name);
  const auto max_it = group_max_positions_.find(group_name);
  if (min_it != group_min_positions_.end() && max_it != group_max_positions_.end() &&
    joint_index < min_it->second.size() && joint_index < max_it->second.size())
  {
    position = std::clamp(position, min_it->second[joint_index], max_it->second[joint_index]);
  }
...
double JointTrajectoryCommandBroadcaster::calculate_mean_error() const
{
  ...
  for (const auto & group_pair : group_joint_names_) {
    ...
    for (size_t i = 0; i < group_joints.size(); ++i) {
      ...
      leader_pos = transform_position(group_name, joint_name, i, leader_pos);
  ...
  for (const auto & group_name : trajectory_groups_) {
    ...
    if (realtime_publisher) {
      ...
      for (size_t i = 0; i < num_joints; ++i) {
        ...
        pos_value = transform_position(group_name, group_joints[i], i, pos_value);
